...
Locked: window reveal = sliding panel + curtain split, duration slaved to the text animation; window content = looped duotone+accent video; color = only inside the window + the asterisk cycles hues while open; timing = springy; mobile = tap toggles; reduced-motion = fully disabled (static hero); layout = reserved slot (zero shift); close = on leave; word order = shuffled per visit. Words: a product manager · a builder · a lifelong learner · a technologist · an Illini · an analyst (article stored with each word). Mark = **sickle six** (curved sharp six-blade) + **whirl five** (hard-capped curved strokes) — primary/variant assignment pending Salman. Name→word transition = **T1 backspace caret** (steady-blink caret as its own object, human-jittered cadence, deletion faster than typing); word cycling = **same mechanic** (delete to pivot, retype). Stack: Motion for React + custom per-letter engine; video not GIF.
